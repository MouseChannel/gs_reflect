...
from super_gaussian_renderer import render, network_gui, render_lighting
import sys
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations):
    ...
    for iteration in range(1, opt.iterations + 1):
        ...
        with torch.no_grad():
            ...
            if iteration % 10 == 0:
                cur_psnr =psnr(image, gt_image)
                progress_bar.set_postfix({"Loss": f"{ema_loss_for_log:.{7}f}" ,"PSNR ":f"{torch.tensor(cur_psnr).mean():.{3}f}" })
            ...
            training_report(tb_writer, iteration, Ll1, loss, losses_extra, l1_loss, iter_start.elapsed_time(iter_end),
            ...
            if iteration < opt.densify_until_iter:
                ...
                if iteration > opt.densify_from_iter and iteration % opt.densification_interval == 0:
                    ...
                    gaussians.densify_and_prune(opt.densify_grad_threshold, 0.005, scene.cameras_extent, size_threshold,1e9)
...
def training_report(tb_writer, iteration, Ll1, loss, losses_extra, l1_loss, elapsed, testing_iterations, scene: Scene,
                ...
                for idx, viewpoint in enumerate(config['cameras']):
                    ...
                    if tb_writer and (idx < 5):
                        ...
                        if renderArgs[0].brdf:
                            ...
                            if tb_writer:
                                tb_writer.add_images(config['name'] + "/lighting", lighting[None],
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[10,7_000,1_000,5_000,8_000,15_000,20_000,30_000])
