...
__device__ glm::vec3 computeColorFromSH(int idx, int deg, int max_coeffs, const glm::vec3* means, glm::vec3 campos, const float* shs, bool* clamped)
{
	...
	glm::vec3* sh = ((glm::vec3*)shs) + idx * (24);
	...
	float2* __restrict__ wichzone)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			glm::vec3* sh = ((glm::vec3*)shs) + collected_id[j] * (16+4+4);
			glm::vec3 addrgb[4],kernal[4];
			addrgb[0] = sh[16+0];
			addrgb[1] = sh[16+1];
			addrgb[2] = sh[16+2];
			addrgb[3] = sh[16+3];
			...
			float eps = -1e-4;
			float exp_scale[4],my_rho[4],opa[4];
			my_rho[0] = (proj_u-kernal[0].x) * (proj_u-kernal[0].x) + (proj_v-kernal[0].y) * (proj_v-kernal[0].y);
			my_rho[1] = (proj_u-kernal[1].x) * (proj_u-kernal[1].x) + (proj_v-kernal[1].y) * (proj_v-kernal[1].y);
			my_rho[2] = (proj_u-kernal[2].x) * (proj_u-kernal[2].x) + (proj_v-kernal[2].y) * (proj_v-kernal[2].y);
			my_rho[3] = (proj_u-kernal[3].x) * (proj_u-kernal[3].x) + (proj_v-kernal[3].y) * (proj_v-kernal[3].y);

			exp_scale[0] = kernal[0].z;
			exp_scale[1] = kernal[1].z;
			exp_scale[2] = kernal[2].z;
			exp_scale[3] = kernal[3].z;
			float4 my_weight;
			my_weight.x = exp(-1.0f*my_rho[0]*exp_scale[0]) - eps;
			my_weight.y = exp(-1.0f*my_rho[1]*exp_scale[1]) - eps;
			my_weight.z = exp(-1.0f*my_rho[2]*exp_scale[2]) - eps;
			my_weight.w = exp(-1.0f*my_rho[3]*exp_scale[3]) - eps;

			opa[0] = my_o.x * my_weight.x;
			opa[1] = my_o.y * my_weight.y;
			opa[2] = my_o.z * my_weight.z;
			opa[3] = my_o.w * my_weight.w;

			float my_opa = opa[0] + opa[1] + opa[2] + opa[3];
			my_opa = my_opa / 4.0f;
			...
			nncolor[0] = my_weight.x * addrgb[0].x +  my_weight.y * addrgb[1].x + my_weight.z * addrgb[2].x + my_weight.w * addrgb[3].x;
			nncolor[1] = my_weight.x * addrgb[0].y +  my_weight.y * addrgb[1].y + my_weight.z * addrgb[2].y + my_weight.w * addrgb[3].y;
			nncolor[2] = my_weight.x * addrgb[0].z +  my_weight.y * addrgb[1].z + my_weight.z * addrgb[2].z + my_weight.w * addrgb[3].z;

			nncolor[0] = nncolor[0] / 4.0f;
			nncolor[1] = nncolor[1] / 4.0f;
			nncolor[2] = nncolor[2] / 4.0f;
